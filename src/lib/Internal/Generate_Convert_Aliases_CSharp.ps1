...
"@)
    ...
    if($instance) {
        ...
        public $($modifier)$csName To$($name)(object value) {
            ...
            return Convert.ToBasicStruct<$csName>(value, this.Options, false, null, this.Converters);
        }
...
"@) 
        ...
        public $($modifier)$csName To$($name)(object value) {
            if(value is $csName $($csCamelName)Value) return $($csCamelName)Value;
            return Convert.ToBasicStruct<$csName>(value, ConvertOptions.Default, false, null, Converter.Default.Converters);
        }
