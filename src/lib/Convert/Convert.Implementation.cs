...
namespace Ockham.Data
{
    ...
    public static partial class Convert
    {
        ...
        internal static object To(object value, Type targetType, ConvertOptions options, bool ignoreError, object valueOnError, IReadOnlyDictionary<Type, ConverterDelegate> converters)
        {
            if (!targetType.IsValueType) return ToReference(value, targetType, options, ignoreError, valueOnError, converters);
            ...
            if (TypeInspection.IsNullableOfT(targetType))
            {
                ...
                if (!ignoreError) return ToStructValue(value, underlyingType, options, false, null, converters);

                try
                {
                    return ToStructValue(value, underlyingType, options, false, null, converters);
                }
                catch
                {
                    return valueOnError;
            ...
            return ToStructValue(value, targetType, options, ignoreError, valueOnError, converters);
        }

        internal static object ToReference(object value, Type targetType, ConvertOptions options, bool ignoreError, object valueOnError, IReadOnlyDictionary<Type, ConverterDelegate> converters)
        {
            ...
            return ToReferenceValue(value, targetType, options, ignoreError, valueOnError, converters);
        }

        private static object ToReferenceValue(object value, Type targetType, ConvertOptions options, bool ignoreError, object valueOnError, IReadOnlyDictionary<Type, ConverterDelegate> converters)
        {
            if (ignoreError)
            {
                try
                {
                    return ToReferenceValue(value, targetType, options, false, valueOnError, converters);
                }
                catch
                {
            ...
            if ((converters != null) && converters.TryGetValue(targetType, out ConverterDelegate converter))
            {
        ...
        private static object ToStructValue(
            ...
            bool ignoreError,
            object valueOnError,
            IReadOnlyDictionary<Type, ConverterDelegate> converters
        )
        {
            if (ignoreError)
            {
                try
                {
                    return ToStructValue(value, targetType, options, false, valueOnError, converters);
                }
                catch
                {
            ...
            if (targetType.IsEnum)
            {
                ...
                if (Value.IsNumeric(value, options))
                {
                    ...
                    var rawValue = ToStructValue(value, Enum.GetUnderlyingType(targetType), options, false, null, converters);
            ...
            if (value is string stringValue)
            {
                if (TypeInspection.IsNumberType(targetType))
                {
                    if (options.ParseBaseN && Value.IsNumeric(stringValue, options.ParseFlage, out int @base) && @base != 10)
                    {
                        ...
                        return ToStructValue(ulongVal, targetType, options, ignoreError, valueOnError, converters);
                    }
                }
            }
